...
dbgapStudyContent <- dbgap[, c("accession", "Study.Content")]
dbgapStudyContent <- dbgapStudyContent[ dbgapStudyContent$accession %in% dbgapSelection$accession, ]
dbgapStudyContent$Study.Content <- as.character( dbgapStudyContent$Study.Content)

dbgapStudyContent$phenotype.dataset <- NA
dbgapStudyContent$variables <- NA
dbgapStudyContent$documents <- NA
dbgapStudyContent$analyses <- NA
dbgapStudyContent$molecular.datasets <- NA
dbgapStudyContent$subjects <- NA
dbgapStudyContent$samples <- NA
...
dbgapStudyContent <- dbgapStudyContent[, c(1, 3:9)]
...
##remove those with less than 30 variables
#finalSetFilter <- finalSet[!is.na( finalSet$variables), ]
#nrow(finalSetFilter) #1135

#finalSetFilter <- finalSetFilter[ as.numeric(finalSetFilter$variables) >= 100, ]
#nrow(finalSetFilter) #154

#finalSetFilter <- finalSetFilter[ as.numeric(finalSetFilter$samples) >= 100, ]
#nrow(finalSetFilter) #280

#finalSetFilter <- finalSetFilter[ as.numeric(finalSetFilter$subjects) >= 1000, ]
#nrow(finalSetFilter) #160

##add the columns that we need and sort the table
finalSetFilter <- finalSet
finalSetFilter$Country <- NA
finalSetFilter$Patients.Age <- NA
finalSetFilter$Notes <- NA
finalSetFilter$PubMedLink <- NA
finalSetFilter$Phenotypic.Data.Type <- NA
...
##check which ones are in TopMED freeze 8
#we download the information from https://www.nhlbiwgs.org/topmed-whole-genome-sequencing-methods-freeze-8
# topmed <- read.delim("/Users/alba/Desktop/biobankPaper/Biobanks/catalogBiobanksTopMED.csv", sep=',')
# 
# for( i in 1:nrow(finalSetFilterSort)){
#   if( finalSetFilterSort$dbGaP.TOPMed.Study.Accession[i] %in% topmed$dbGaP.TOPMed.Study.Accession ){
#     finalSetFilterSort$Notes[i] <- "TOPMed Freeze 8"
#   }
#   if( finalSetFilterSort$dbGaP.TOPMed.Study.Accession[i] %in% topmed$dbGaP.Parent.Study.Accession ){
#     finalSetFilterSort$Notes[i] <- "TOPMed Freeze 8"
#   }
# }
# 
# finalSetFilterSort <- finalSetFilterSort[, c(1:17)]
# colnames(finalSetFilterSort) <- c("Name", "Country", "Sample Size", "Subject Count", "Study Design",
#                                   "Phenotypic Variables", "Phenotypic Data Type", "Molecular Data Type", "Markerset", 
#                                   "Disease/Focus", "Patients Age", "Ancestry", 
#                                   "Consent", "Accession", "Link", "PubMedLink", "Notes")
# 
# 
# finalSetFilterSort$Name <- gsub( ",", ";", finalSetFilterSort$Name)
# finalSetFilterSort$`Molecular Data Type` <- gsub( ",", ";", finalSetFilterSort$`Molecular Data Type`)
# finalSetFilterSort$`Disease/Focus` <- gsub( ",", ";", finalSetFilterSort$`Disease/Focus`)
...
# 
# write.table( finalSetFilterSort, file=paste0(file_path,"dbgapInfoUpdated.csv"), 
#              sep = ',', 
#              col.names = TRUE, 
#              row.names = FALSE, 
#              quote = FALSE)
...
f5b = data.table(finalSetFilterSort) # freeze5bExtensive
...
f5b = f5b[,Country:="USA"]# Registry cohort?? Maybe? [, Phenotypic_Data_Type:="TBD"] 
f5b = f5b[, Accession:="Request access through dbGAP: https://dbgap.ncbi.nlm.nih.gov/aa/wga.cgi"]
pubMed="https://www.ncbi.nlm.nih.gov/pubmed?cmd=DetailsSearch&term="
...
setValue("FHS", "PubMedLink", paste0(pubMed,"5921755[PMID]"))
...
setValue("MESA", "PubMedLink",  paste0(pubMed,"12397006[PMID]"))
...
setValue("ARIC", "PubMedLink",  paste0(pubMed,"2646917[PMID]"))
...
setValue("CHS", "PubMedLink",  paste0(pubMed,"1669507[PMID]"))
...
setValue("JHS", "PubMedLink",  paste0(pubMed,"10100686[PMID]"))
...
setValue("CFS", "PubMedLink",  paste0(pubMed,"10228121[PMID]"))
...
setValue("GENOA", "PubMedLink",  paste0(pubMed,"15121494[PMID]"))
...
setValue("COPDGene", "PubMedLink",  paste0(pubMed,"20214461[PMID]"))
...
setValue("SAS", "PubMedLink",  paste0(pubMed,"24799123[PMID]"))
setValue("SAS", "Phenotypic_Data_Type", "TBD")

# HyperGEN - phs001293 (geno) 
# Ancestry: https://www.ncbi.nlm.nih.gov/projects/gap/population/cgi-bin/StudySubjectAncestry.cgi?phs=954&version=2&exp1=1&exp2=1&exp3=1
setValue("HyperGEN","Patients_Age", "<60")
setValue("HyperGEN", "PubMedLink",  paste0(pubMed,"11115379[PMID]"))
...
setValue("GeneSTAR", "PubMedLink",  paste0(pubMed,"16551714[PMID]"))
...
setValue("GOLDN", "PubMedLink",  paste0(pubMed,"22228203[PMID]"))
