...
loadData <- function() {
  ...
  dataNotValidated <- lapply(filePaths, drop_read_csv, stringsAsFactors = FALSE)
  # Concatenate all data together into one data.frame
  dataNotValidated <- data.frame(matrix(unlist(dataNotValidated), nrow=length(dataNotValidated), byrow=T))
  colnames( dataNotValidated ) <- c("Name", "email","afiliation",
                                    "Biobank", "Country", "Number Of Samples", "Number Of Patients",
                                    "Genomic data", "Phenotypic Data", "Disease Specific" )
  
  dataNotValidated <- dataNotValidated[, c("Biobank", "Country", "Number Of Samples", "Number Of Patients",
                                           "Genomic data", "Phenotypic Data", "Disease Specific" )]
  dataNotValidated$Validation <- "Pending"
  dataNotValidated
}
